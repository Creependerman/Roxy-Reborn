...
public class RapeMasterFontManager extends FontRenderer {
    ...
    public void drawRightAlignedString(String s, float x, float y, int color) {
        drawString(s, x - getStringWidth(s), y, color);
    }

    public void drawRightAlignedStringWithShadow(String s, float x, float y, int color) {
    ...
    private int generateCharTexture(int id) {
        ...
        for (int y = 0; y < 16; y++)
            for (int x = 0; x < 16; x++) {
                char chaR = (char) ((y << 4 | x) | offset);
                String chr = String.valueOf(chaR);
                if (!font.canDisplay(chaR)) {
                    font = new Font("default", Font.PLAIN, font.getSize());
                }
                g.setFont(font);
